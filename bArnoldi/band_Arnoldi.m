function result = band_Arnoldi(nmax,R,MultA,tol,n0,result0)
%
...
%
  ...
  for k = 1 : mc -1,
	...
	if tmp ~= 0 
		flops = flops + size(V,1);          % flop count added by Efrem Rensi 3/2014
	end
...
%
  tmpv = MultA(V(:,n));
  flops = flops + size(V,1)^2;  % flop-count added by Efrem Rensi
...
%
  for k = 1 : n,
    H(k,n) = (V(:,k))' * tmpv;
	if H(k,n) ~= 0
		flops = flops + size(V,1);  % flop-count added by Efrem Rensi
	end
    tmpv   = tmpv - V(:,k) * H(k,n);
  end
%
